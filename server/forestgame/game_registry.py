...
class Game:
  ...
  def get_world(self):
    ...
    building_db_results = self.db.query("SELECT x, y, building_id, owner_id FROM world_building WHERE world_uuid=%s", (self.world_uuid, ))
    buildings = [{"in_db": True, "x": b[0], "y": b[1], "id": b[2], "owner_id": b[3]} for b in building_db_results]

    row = db_result[0]
    return World(self.db, self.world_uuid, row[0], row[1], row[2], tile_db_results, buildings)
